...
![Solved Cases Gauge](plot_image.png)
...
unsolved_data <- data %>% filter(Solved == "No")

unsolved_counts <- unsolved_data %>%
  group_by(State) %>%
  summarise(unsolved_homicides = n())

state_data <- data.frame(state = tolower(unsolved_counts$State), 
                         unsolved_homicides = 
                           unsolved_counts$unsolved_homicides)
...
9. Stacked Area Chart for Weapon used over the years in solved and unsolved cases

```{r stacked-area, message=FALSE, warning=FALSE}
# weapon over years
# Plot the stacked area chart
# Group data by Year and Weapon and count occurrences

my_custom_colors <- c(
  "Handgun - pistol, revolver, etc" = "#1f77b4",  # Dark blue
  "Knife or cutting instrument" = "lightsalmon",      # Orange
  "Firearm, type not stated" = "darkseagreen3",         # Green
  "Personal weapons, includes beating" = "tomato4", # Red
  "Shotgun" = "#9467bd",                          # Purple
  "Other or type unknown" = "peru",#"#8c564b",            # Brown
  "Rifle" = "#e377c2",                            # Pink
  "Blunt object - hammer, club, etc" = "azure3", # Gray
  "Strangulation - hanging" = "#bcbd22",          # Olive
  "Fire" = "gold",#"hotpink4",                             # Cyan
  "Asphyxiation - includes death by gas" = "darkblue", # Light blue
  "Narcotics or drugs, sleeping pills" = "purple",#"#ffbb78",   # Light orange
  "Other gun" = "red",                        # Light green
  "Drowning" = "darkorchid4",                         # Light red
  "Weapon Not Reported" = "gray0",              # Light purple
  "Poison - does not include gas" = "#c49c94",    # Light brown
  "Explosives" = "#f7b6d2",                       # Light pink
  "Pushed or thrown out window" = "#c7c7c7"       # Light gray
)


# Filter data for Solved = "Yes"
weapon_usage_solved_yes <- data %>%
  filter(Solved == "Yes") %>%
  group_by(Year, Weapon) %>%
  summarise(count = n()) %>%
  ungroup()

# Filter data for Solved = "No"
weapon_usage_solved_no <- data %>%
  filter(Solved == "No") %>%
  group_by(Year, Weapon) %>%
  summarise(count = n()) %>%
  ungroup()

ggplot(weapon_usage_solved_yes, aes(x = Year, y = count, fill = Weapon)) +
  geom_area(alpha = 0.6) +
  labs(title = "Weapon Usage Over Years For Solved Cases", 
       x = "Year", y = "Count of Incidents") +
...
ggplot(weapon_usage_solved_no, aes(x = Year, y = count, fill = Weapon)) +
  geom_area(alpha = 0.6) +
  labs(title = "Weapon Usage Over Years For Unsolved Cases",
       x = "Year", y = "Count of Incidents") +
  scale_fill_manual(values = my_custom_colors)+
  #scale_fill_brewer(palette = "Set3")+
  theme(panel.grid.minor = element_blank(),
